...
class ZHist:
    def __init__(self, data, drop_cols, distance="chi2", minTrend = 300, 
                 ...
                 matrix_cols = [],
                 removeMatrix = False, removeTrend = True):
        self.data = data
            ...
            for v2 in v1:
                ...
                if (v2[0][1] not in letter) and (self.removeMatrix == False or (self.removeMatrix == True and v2[0][0] not in self.matrix_cols)):
                    if self.removeTrend == False:
                        if v2[0][1] in ['a', 'c'] or (v2[0][1] in ['i','d'] and v2[2] - v2[1] >= self.minTrend):
                            tmparr.append(v2)
    ...
    def fit(self):
        ...
        self.repair_intervals_removed = self.removeNormal(self.repair_intervals, ['b','s'])
        self.normal_intervals_removed = self.removeNormal(self.normal_intervals, ['b','s'])