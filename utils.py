...
def getRelationFull(A, B, constraints):
    relation = None
...
def getRelation(A, B, constraints):
    ...
    if A.end >= B.end:
        ...
        relation = "o"
    elif B.start - A.end < constraints["gap"]:
        relation = "b"
        ...
        for E in FL1[k]:
            for R in FL1[k][E]:
                if k not in FL2 or E not in FL2[k] or R not in FL2[k][E]:
                    ...
                    "relSup1": (FL1[k][E][R])/(n1), "relSup2": 0,
                    "disprop": (((FL1[k][E][R]))/(n1))/(1/(n2))})
                else:
                    F.append({"events": E, "relations": R, "freq1": FL1[k][E][R], "freq2": FL2[k][E][R],
                    "relSup1": (FL1[k][E][R])/(n1), "relSup2": (FL2[k][E][R])/(n2),
                    "disprop": ((FL1[k][E][R])/(n1))/((FL2[k][E][R])/(n2))})
